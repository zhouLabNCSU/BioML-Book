...
save(y.sample.matrix,y.test.matrix,rf.pred.matrix,xgboost.pred.matrix,svm.pred.matrix,lasso.pred.matrix,ridge.pred.matrix,enet.pred.matrix,knn.pred.matrix,neural.pred.matrix,lda.pred.matrix,hclust.pred.matrix,kmeans.pred.matrix,file="microbiome/paper/MicrobiomeHD_ob_goodrich_predict.Rdata")


load("microbiome/paper/MicrobiomeHD_ob_goodrich_predict.Rdata")
...
png(file="microbiome/paper/MicrobiomeHD_ob_goodrich.png")
plot(rf$specificities,rf$sensitivities,type="n",xlim=c(1,0),xlab="Specificity",ylab="Sensitivity",main="Goodrich (135/279 x 11,225)")
...
abline(1,-1,col="gray")
legend("bottomright",c("RF (0.66)","Gboost (0.72)","SVM (0.61)","Lasso (0.56)","Ridge (0.65)","Enet (0.66)","k-NN (0.59)","Neural (0.58)","LDA (0.51)"),col=c("black","blue","red","green","orange","purple","brown","pink","gold"),lty=1)
dev.off()
...
png(file="microbiome/paper/MicrobiomeHD_ob_goodrich_auc.png")
plot(original[1],hfe[1],xlim=c(0.5,1),ylim=c(0.5,1),xlab="All 11,225 OTUs",ylab="50 informative features from HFE",main="Goodrich AUC",pch=19,cex=2,col="black")
par(new=T)
