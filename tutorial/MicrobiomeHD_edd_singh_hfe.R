...
metadata <- read.table("tutorial/edd_singh.metadata.txt",sep="\t",header=T,row.names=1,stringsAsFactors=F)
otu_table <- read.table("tutorial/edd_singh.otu_table.100.denovo.rdp_assigned",sep="\t",header=T,row.names=1)
metadata <- subset(metadata, Time.Point==1)
#vector of discrete trait; input labels file for HFE
y <- t(metadata$DiseaseState)
rownames(y) <- "label"
write.table(y,"microbiome/HFE/edd_singh_label_vector.tab",sep="\t",row.names=T,col.names=F,quote=F)
...
write.table(tax_table,"microbiome/HFE/edd_singh_tax_table.tab",sep="\t",row.names=F,col.names=F,quote=F)
#input OTU table file for HFE
rownames(otu_table) <- tax_table[,1]
write.table(otu_table,"microbiome/HFE/edd_singh_otu_table.tab",sep="\t",row.names=T,col.names=F,quote=F)

#output file from HFE
data <- read.table("tutorial/edd_singh_Final_feature_set.csv",sep=",",header=T)
#vector of discrete trait coded 1=EDD cases and 0=Healthy controls
y <- as.numeric(data$label=="EDD")
...
save(y.sample.matrix,y.test.matrix,rf.pred.matrix,xgboost.pred.matrix,svm.pred.matrix,lasso.pred.matrix,ridge.pred.matrix,enet.pred.matrix,knn.pred.matrix,neural.pred.matrix,lda.pred.matrix,hclust.pred.matrix,kmeans.pred.matrix,file="microbiome/paper/MicrobiomeHD_edd_singh_hfe.Rdata")
...
png(file="microbiome/paper/MicrobiomeHD_edd_singh_hfe.png")
plot(rf$specificities,rf$sensitivities,type="n",xlim=c(1,0),xlab="Specificity",ylab="Sensitivity",main="Singh (201/82 x 31 informative features from HFE)")
...
abline(1,-1,col="gray")
legend("bottomright",c("RF (0.96)","Gboost (0.94)","SVM (0.93)","Lasso (0.94)","Ridge (0.94)","Enet (0.94)","k-NN (0.91)","Neural (0.94)","LDA (0.92)"),col=c("black","blue","red","green","orange","purple","brown","pink","gold"),lty=1)
dev.off()
