...
save(y.sample.matrix,y.test.matrix,rf.pred.matrix,xgboost.pred.matrix,svm.pred.matrix,lasso.pred.matrix,ridge.pred.matrix,enet.pred.matrix,knn.pred.matrix,neural.pred.matrix,lda.pred.matrix,hclust.pred.matrix,kmeans.pred.matrix,file="microbiome/paper/MicrobiomeHD_edd_singh_genus.Rdata")


#calculate average predicted y by sample
load("microbiome/paper/MicrobiomeHD_edd_singh_genus.Rdata")
for (n in 1:numsets) {
...
png(file="microbiome/paper/MicrobiomeHD_edd_singh_genus.png")
plot(rf$specificities,rf$sensitivities,type="n",xlim=c(1,0),xlab="Specificity",ylab="Sensitivity",main="Singh (201/82 x 122 OTUs collapsed at genus level)")
...
abline(1,-1,col="gray")
legend("bottomright",c("RF (0.95)","Gboost (0.94)","SVM (0.92)","Lasso (0.94)","Ridge (0.93)","Enet (0.94)","k-NN (0.89)","Neural (0.96)","LDA (0.89)"),col=c("black","blue","red","green","orange","purple","brown","pink","gold"),lty=1)
dev.off()
